...
5. 执行 §5.1 的 16 格比较，结果追加到本件 §5.1 之下，本节的执行结果见 §9。
6. 实施 §3.4 的豁免收口

---

## 9 §5.1 的执行结果（2026-08-02 追加）
...
确认所比较的正是判据注册时所指的那批 run。归档侧 `P_R' = +0.11 [-1.35, +1.57]`。
...
| GEO_DIV | 差 | -0.03 | +0.01 | -0.07 | -0.00 |
| GEO_SG1 | 复算 | 74.41 | 74.17 | 73.36 | 73.41 |
| GEO_SG1 | 归档 | 74.41 | 74.12 | 73.32 | 73.39 |
| GEO_SG1 | 差 | +0.01 | +0.05 | +0.04 | +0.02 |
| RND_D1 | 复算 | 63.07 | 62.51 | 63.72 | 63.03 |
| RND_D1 | 归档 | 63.08 | 62.48 | 63.73 | 62.95 |
| RND_D1 | 差 | -0.00 | +0.03 | -0.01 | +0.09 |
| RND_S1 | 复算 | 62.78 | 62.52 | 62.40 | 63.82 |
| RND_S1 | 归档 | 62.81 | 62.75 | 62.42 | 63.80 |
| RND_S1 | 差 | -0.04 | -0.24 | -0.02 | +0.02 |

差值在**未舍入值**上形成，仅在显示层舍入。因此某格的显示差值可能与其上方两行
显示值之差相差 0.01，这符合 `P0B_PREREG_ANALYSIS_PLAN` §3，不是算术错误。

无一格逐位重现（符合 §5.1 其他约束第 1 条的预期）。16 个差跨 `-0.24` 到 `+0.09`，
15 个绝对值低于 0.10。最大者为 `RND_S1` seed 1 的 `-0.24`，该格解释了
`P_R'` 由归档侧 `+0.11` 移至复算侧 `+0.21` 的大部分。

**依 §5.1，不为 C 补设阈值。** 本研究没有"纯 GPU 非确定性下重跑同一配置的差异幅度"
的基线测量，故上述差值无法在任何方向上拆分为代码导致的部分与非确定性导致的部分。
判据 C 的逐格绝对值比较在原理上比对比更敏感（对比可能保持而绝对水平整体平移），
但缺乏该基线使这一敏感性无法兑现。
...
本节全部数值由 `mamba_scan_study/analysis/inertness_check_16.py` 生成，
该脚本为只读，按 `P0B_PREREG_ANALYSIS_PLAN` §2/§3 的口径实现，
并内置归档侧自洽性校验（归档 `P_G'` 必须重现 §5.1 的记录值 `+3.50 [2.14, 4.86]`，
否则拒绝输出判定）。用法：

    python3 mamba_scan_study/analysis/inertness_check_16.py

退出码 0 表示 A 与 B 均通过，1 表示判据不通过，2 表示数据缺失或自洽性校验失败。

**本节初版（commit `873e2bb`）的差值表由已舍入的显示值相减得出，
与 §3 口径不符，六格差 0.01，跨度记为 `-0.23` 到 `+0.08`。
现按脚本输出更正。判据 A 与 B 的结论、`P_G'`、`P_R'` 的点估计与区间均未改变。**